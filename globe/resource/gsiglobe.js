...
CONFIG.layers = [
	...
	{
		"url":'../layers_txt/layers_antarctic.txt'
	},	
	{
...
GSI.TEXT.EVAC.KIYAKULINK = '<a href="http://www.gsi.go.jp/bousaichiri/hinanbasho.html" target="blank">「指定緊急避難場所」について</a>　<a href="https://hinan.gsi.go.jp/hinanjocjp/hinanbasho/koukaidate.html" target="blank">市町村別公開日・更新日一覧</a>';
		...
		//try{
			...
			for(var i=0; i<len; i++){
				...
				if(geomType == "Point" && (markerType == "Icon" || markerType == "DivIcon")){
					...
					if ( depthFlag )
					{
						var cartographic = Cesium.Cartographic.fromCartesian(position, this.viewer.scene.ellipsoid);
						if ( cartographic.height < 0 ) continue;
					}
